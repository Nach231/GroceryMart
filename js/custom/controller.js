...
function checkuser(json, key) {
    ...
    if (flag) {
        console.log('Autenticación exitosa. Guardando usuario en la base de datos...');
        document.getElementById("logindiv").style.display = "none";
        document.getElementById("homediv").style.display = "block";

        // Realizar la llamada AJAX para guardar el usuario en la base de datos
        const username = document.querySelector("#username").value;
        const url = 'http://localhost:3000/guardar-usuario'; // Ruta en tu servidor para guardar el usuario en la base de datos
        
        console.log("Guardando usuario en la base de datos...")
        ajaxcall.ajax(function (response) {
            // Aquí puedes manejar la respuesta del servidor si es necesario
            console.log('Respuesta del servidor:', response);
        }, url, username);

        ajaxcall.ajax(printMainMenu, "http://localhost:3000/json/menu.json", "mainmenu");
        ajaxcall.ajax(printSubMenu, "http://localhost:3000/json/Vegetables.json", "submenu");
    } else {
